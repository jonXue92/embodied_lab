...
const curriculum: CurriculumUnit[] = [
  ...
  },
  {
    ...
    sources: [{ label: 'LeRobot Datasets', url: 'https://huggingface.co/docs/lerobot/en/il_robots' }, { label: 'DROID Dataset', url: 'https://droid-dataset.github.io/' }, { label: 'Stanford BEHAVIOR 2026 Updates', url: 'https://behavior.stanford.edu/challenge/updates.html' }],
    days: [
      ['Episode、frame 与 transition 数据模型', '读取 LeRobotDataset 的样本结构', '数据模式如何限制可训练任务'],
      ['图像、关节状态与动作的时间对齐', '用 BEHAVIOR-1K 速度字段修订案例实现 timestamp/frame 审计', '数据字段修复为何会让旧 checkpoint 结论失效'],
      ...
      ['数据版本、切分与泄漏防护', '生成 LeRobot/BEHAVIOR train-eval manifest', '怎样让跨数据集模型对比结论可审计'],
    ],
  },
  {
    ...
    sources: [{ label: 'Diffusion Policy', url: 'https://diffusion-policy.cs.columbia.edu/' }, { label: 'ACT', url: 'https://tonyzhaozh.github.io/aloha/' }, { label: 'BEHAVIOR 2026 Challenge', url: 'https://behavior.stanford.edu/challenge/index.html' }],
    days: [
      ...
      ['策略族公平对比方法', '在 BEHAVIOR 基线协议中统一数据、动作空间与评测脚本', '如何避免用任务实例和配置差异冒充算法提升'],
    ],
  },
  {
    title: '视觉语言基础', outcome: '读懂视觉编码、语言条件和多模态融合如何进入机器人策略。', deliverable: '多模态 token 流程图 + 输入消融实验', keywords: ['视觉', '语言', 'token', '对齐', '编码器', '融合'],
    sources: [{ label: 'Transformers', url: 'https://huggingface.co/docs/transformers/index' }, { label: 'OpenVLA', url: 'https://openvla.github.io/' }, { label: 'World Labs Atlas', url: 'https://www.worldlabs.ai/blog/atlas' }],
    days: [
      ['ViT patch 与视觉表征', '检查 patch embedding、dense feature 与特征尺度', '从 ImageNet 物体识别到李飞飞团队空间智能主张还缺什么'],
      ...
      ['多视角相机融合与空间一致性', '实现 camera mask、视角消融和简化相机位姿条件', 'World Labs Atlas 为什么把多视角输入提升为共享空间上下文'],
  ...
  },
  {
    ...
    sources: [{ label: 'Isaac GR00T', url: 'https://github.com/NVIDIA/Isaac-GR00T' }, { label: 'LingBot-VLA', url: 'https://github.com/Robbyant/lingbot-vla' }, { label: 'Meta V-JEPA 2', url: 'https://ai.meta.com/research/vjepa/' }],
    days: [
      ...
      ['四模型与 V-JEPA 表征统一横评', '生成参数、数据、动作、规划与时延表', '如何区分直接动作策略和 LeCun 团队的预测式世界模型'],
    ],
  },
  {
    title: 'World Model：空间生成与 JEPA 规划', outcome: '比较李飞飞团队的空间生成/仿真路线与 LeCun 团队的表征预测/规划路线，并设计快慢系统协作。', deliverable: 'World Labs—JEPA 双路线架构图 + 最小规划实验', keywords: ['world model', '空间智能', 'JEPA', '未来', '规划', '仿真'],
    sources: [{ label: 'World Labs Atlas', url: 'https://www.worldlabs.ai/blog/atlas' }, { label: 'World Labs R2S2R', url: 'https://www.worldlabs.ai/blog/real-to-sim-to-real' }, { label: 'Meta V-JEPA 2', url: 'https://ai.meta.com/research/vjepa/' }, { label: 'V-JEPA 2.1', url: 'https://arxiv.org/abs/2603.14482' }, { label: 'AMI Labs', url: 'https://amilabs.xyz/' }],
    days: [
      ['World Model 的四种职责：renderer、simulator、planner 与闭环模型', '实现统一 observe-predict-act 最小接口', '李飞飞与 LeCun 两条路线在输出对象上有何不同'],
      ['像素/3D 生成与 latent 表征预测', '比较 Atlas/Cosmos 生成目标和 JEPA embedding loss', '视觉逼真与控制有效分别需要什么证据'],
      ['V-JEPA 2/2.1 的自监督视频表征', '追踪 context encoder、target encoder、predictor 与 dense feature', '为什么预测抽象表征可能忽略不可预测细节'],
      ['V-JEPA 2-AC 与 JEPA-WM 物理规划', '实现 latent action-conditioned rollout 和 goal cost', '不到 62 小时 DROID 视频的结论边界是什么'],
      ['World Labs Atlas 与 R2S2R', '拆解多模态空间上下文、Real-to-Sim 变体和传感器视角', '生成世界怎样成为机器人训练与评测引擎'],
      ['Dreamer、JEPA 与快慢规划', '实现异步计划缓存、value target 和过期检查', '2 秒世界模型如何服务 50 ms policy'],
      ['双路线评测与反例', '设计 BEHAVIOR、物理推理、预测和真机闭环指标', '为什么单看视频质量或单个抓取成功率都会误导'],
    ],
  },
  {
    title: '仿真与 Sim2Real', outcome: '用仿真形成安全、可规模化且可迁移的实验闭环。', deliverable: 'SO-101 仿真—真机差距清单', keywords: ['仿真', 'sim2real', '随机化', '标定', '碰撞', '评测'],
    sources: [{ label: 'Isaac Lab', url: 'https://isaac-sim.github.io/IsaacLab/' }, { label: 'OmniGibson', url: 'https://behavior.stanford.edu/' }, { label: 'BEHAVIOR Evaluation', url: 'https://behavior.stanford.edu/challenge/evaluation.html' }, { label: 'World Labs R2S2R', url: 'https://www.worldlabs.ai/blog/real-to-sim-to-real' }],
    days: [
      ['URDF、关节与坐标系', '检查 joint limit 与 transform tree', '模型描述错误如何污染所有实验'],
      ['OmniGibson 与 BEHAVIOR-1K 环境结构', '追踪 BDDL task、reset、step、observation 与 object state', '人类需求驱动的 1000 个任务和短程抓取基准有何差别'],
      ['交互物理、碰撞与控制频率', '记录刚体、可变形物、液体的 contact/state 与 substep', '高视觉/物理真实度仍缺哪些现实证据'],
      ['Domain Randomization 与 World Labs R2S2R', '实现外观、物理、机器人状态和相机视角变体表', '一项真实任务怎样扩展为可控仿真经验'],
      ['系统辨识与参数校准', '拟合摩擦、延迟和执行器响应', '何时该校准、何时该随机化'],
      ['仿真数据混入真机训练', '配置数据来源标签和采样权重', '合成数据如何避免压过真实信号'],
      ['BEHAVIOR Challenge 与 Sim2Real 评测协议', '生成公开/隐藏实例、Q-score 与真机指标对照', '迁移成功需要哪些可重复证据'],
    ],
  },
  {
  ...
  },
  {
    ...
    days: [
      ...
      ['Value、Q 与 advantage', '实现 GAE 并与 action-conditioned world model cost 对照', 'model-free 价值与 LeCun 路线的预测代价有何区别'],
  ...
  },
  {
    ...
    sources: [{ label: 'BEHAVIOR Evaluation', url: 'https://behavior.stanford.edu/challenge/evaluation.html' }, { label: 'V-JEPA 2 Benchmarks', url: 'https://ai.meta.com/blog/v-jepa-2-world-model-benchmarks/' }, { label: 'NIST AI RMF', url: 'https://www.nist.gov/itl/ai-risk-management-framework' }],
    days: [
      ['成功定义、BDDL 谓词与任务容差', '实现 binary success 与 BEHAVIOR Q-score', '部分完成分数何时比二元成功更诚实'],
      ['离线指标与闭环指标', '关联 action error 和 task success', '为什么 L1 更低可能真机更差'],
      ['仿真非确定性与置信区间', '计算重复 rollout 区间和样本量', '为什么不能跨实例挑最好一次'],
      ['隐藏实例与物理推理测试', '构造 BEHAVIOR 扰动网格和 V-JEPA 反事实/预判探针', '泛化应覆盖任务、环境还是动力学'],
      ['安全指标与近失事件', '记录越界、急停和人工介入', '没有损坏不等于没有风险'],
      ['数据修订与回归测试', '复现 BEHAVIOR velocity/depth 修复并生成旧任务回放清单', '数据版本变化为什么必须触发重评'],
      ['双路线评测报告与证据链', '输出生成世界、latent planning、策略闭环和统计摘要', '怎样公平比较 World Labs 与 JEPA 式主张'],
    ],
  },
  {
    title: 'SO-101 毕业项目', outcome: '完成一个可复现、可评测、可讲解的真机持续改进作品。', deliverable: '演示—rollout—介入—改进完整作品', keywords: ['SO-101', '项目', '基线', '实验', '部署', '复现'],
    sources: [{ label: 'LeRobot SO-101', url: 'https://huggingface.co/docs/lerobot/en/so101' }, { label: 'BEHAVIOR 2026', url: 'https://behavior.stanford.edu/challenge/index.html' }, { label: 'JEPA-WMs', url: 'https://github.com/facebookresearch/jepa-wms' }],
    days: [
      ['项目任务与成功标准', '把 SO-101 任务映射为 BDDL 式初始/目标谓词', '选择任务时如何控制风险与工作量'],
      ['硬件装配、标定与安全', '运行电机、相机和限位检查', '硬件可靠性是算法实验前提'],
      ['演示数据采集', '记录可映射 LeRobot/BEHAVIOR 的成功轨迹与 frame 身份', '多少数据才足以建立基线'],
      ['BC/ACT 基线与 JEPA 特征探针', '跑通训练、冻结表征 probe、评测和 checkpoint', '世界模型特征是否真的改善小数据控制'],
  ...
  },
  {
    ...
    sources: [{ label: 'World Labs Research', url: 'https://www.worldlabs.ai/blog' }, { label: 'AMI Labs', url: 'https://amilabs.xyz/' }, { label: 'GitHub Docs', url: 'https://docs.github.com/en/repositories' }],
    days: [
      ['最终作品的 README 与证据链', '验证环境、数据、复现命令和演示链接', '如何让招聘者快速判断项目可信度'],
      ['四个月毕业项目技术答辩', '运行最终回归、指标汇总与演示脚本', '如何回答架构选择、失败案例和安全追问'],
      ['2027 Q1 主攻路线规划', '建立 VLA、World Labs 空间仿真、JEPA/AMI 规划与真机 RL backlog', '如何用现有证据选择下一阶段方向'],
