...
function load(file) {
  ...
  const req = (name) => name === 'cloudflare:workers' ? { env: { DB } } : name.startsWith('@/') ? load(name.slice(2) + '.ts') : name.startsWith('.') ? load(path.resolve(path.dirname(file), name + '.ts')) : require(name);
...
(async () => {
  ...
  for (const id of ids) {
    const { lesson, task, plan } = c.getLessonById(id);
    ...
    for (const key of ['outcomes', 'sections', 'points', 'sources']) assert.ok(lesson[key]?.length >= 2, `${id}: ${key}`);
    ...
    assert.ok(plan.question.title && plan.question.reference && plan.question.summary.includes(plan.tasks[1].title));
  ...
  console.log(JSON.stringify({ days: 122, lessons: ids.length, questions: 122, focusDates: ['2026-09-03','2026-09-04'].map((date) => c.getDailyLearningPlan(date).tasks.map((t) => t.title)), midnightAndResume: 'passed', strictCheckIns: 'passed', migrations: journal.entries.length, preservedTables: protectedTables }, null, 2));
  db.close();
